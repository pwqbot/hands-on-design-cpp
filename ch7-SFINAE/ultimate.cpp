...
struct is_valid_helper {
    ...
    template <typename... LambdaArgs>
    constexpr auto operator()(LambdaArgs &&...args) {
...
auto is_addable = is_valid([](auto &&x) constexpr->decltype(x + x){});
...
auto is_assignable = is_valid(is_assignable_lambada{});
...
auto is_default_constructible = is_valid(is_constructible_lambada{});
